...
public:
	...
	void setMonr(double x, double y, double z, double heading_rad, double lateral_m_s, double lonitudinal_m_s) {
	...
	while (1) {
		...
		if (state == "Disarmed") {
			...
			for (int i = 0; i < traj.size() - 1; ++i) {
				...
				obj.setMonr(currentTraj.pos.xCoord_m,
							currentTraj.pos.yCoord_m,
							currentTraj.pos.zCoord_m,
							currentTraj.pos.heading_rad,
...
/**
 * @brief ISO-object that can be used with dynamic trajectories. The ISO-object works in the same way as
 * runFollowTrajectory, but it will get a new trajectory at runtime, instead of the full trajectory when connecting.
 *
 */
