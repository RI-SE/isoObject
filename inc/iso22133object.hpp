...
public:
	...
	void setPosition(const CartesianPosition& pos) {
		position = pos;
	}
	void setSpeed(const SpeedType& spd) {
		speed = spd;
	}
	void setObjectSettings(const ObjectSettingsType& osem) {
		objectSettings = osem;
	}
	void setAcceleration(const AccelerationType& acc) {
		acceleration = acc;
	}
	void setDriveDirection(const DriveDirectionType& drd) {
		driveDirection = drd;
	}
	void setObjectState(const ObjectStateID& ost) {
		objectState = ost;
	}
	void setName(const std::string nm) {
		name = nm;
	}
	void setReadyToArm(const int& rdy) {
		readyToArm = rdy;
	}
	...
	CartesianPosition getPosition() const {
		return position;
	}
	...
	DriveDirectionType getDriveDirection() const {
		return driveDirection;
	}
	TrajectoryHeaderType getTrajectoryHeader() const {
		return trajDecoder.getTrajHeader();
	}
	std::vector<TrajectoryWaypointType> getTrajectory() const {
		return trajDecoder.getTraj();
	}
	...
	void startHandleTCP() {
		tcpReceiveThread = std::thread(&TestObject::receiveTCP, this);
	}
	void startHandleUDP() {
		udpReceiveThread = std::thread(&TestObject::receiveUDP, this);
	}
	void startHEABCheck() {
		heabTimeoutThread = std::thread(&TestObject::checkHeabLoop, this);
	}
	void startSendMonr() {
		monrThread = std::thread(&TestObject::sendMonrLoop, this);
	}
...
protected:
	...
	virtual void handleAbort() {
		throw std::logic_error("Use of unimplemented abort handler");
	}
...
private:
	...
	char getNextSentMessageCounter() {
		return sentMessageCounter = (sentMessageCounter + 1) % 256;
	}
	...
	void checkAndUpdateMessageCounter(const char receivedMessageCounter) {
		if (receivedMessageCounter != expectedMessageCounter) {
			std::cout << "Message counter mismatch. Expected: " << (int)expectedMessageCounter
					  << " Received: " << (int)receivedMessageCounter << std::endl;
		}
