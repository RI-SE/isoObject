...
void TestObject::sendMONR(bool debug) {
	...
	auto nBytesWritten = encodeMONRMessage(this->populateMessageHeader(&header),
										   &time,
										   this->position,
										   this->speed,
										   this->acceleration,
										   this->driveDirection,
										   this->state->getStateID(),
										   this->readyToArm,
										   this->errorState,
										   0x0000,
										   buffer.data(),
...
void TestObject::sendGREM(HeaderType msgHeader, GeneralResponseStatus responseCode, bool debug) {
	...
	MessageHeaderType header;
	auto nBytesWritten =
	  encodeGREMMessage(this->populateMessageHeader(&header), &grem, buffer.data(), buffer.size(), debug);

	if (nBytesWritten < 0) {
		throw(std::invalid_argument("Failed to encode GREM data"));
	}
...
void TestObject::sendMonrLoop() {
	while (this->on) {
		...
		if (ctrlChannel.isOpen() && !awaitingFirstHeab && this->transmitterID != TRANSMITTER_ID_UNAVAILABLE_VALUE) {
			sendMONR();
		}
...
void TestObject::handleHEAB(HeabMessageDataType& heab) {
	...
	if (networkDelay > maxSafeNetworkDelay) {
		std::stringstream ss;
		ss << "Network delay of " << duration_cast<milliseconds>(networkDelay).count() << " ms exceeds safe limit of "
		   << maxSafeNetworkDelay.count() << " ms." << std::endl;
		std::cerr << ss.str();
		// TODO: do something
	}
	std::scoped_lock lock(heabMutex);
