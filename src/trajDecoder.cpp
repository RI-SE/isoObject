...
ssize_t TrajDecoder::DecodeTRAJ(std::vector<char>& dataBuffer, bool debug) {
	...
	if (!expectingTRAJPoints) {
		...
		tmpByteCounter = decodeTRAJMessageHeader(&this->trajecoryHeader, copiedData.data(), copiedData.size(), debug);
		if (tmpByteCounter < 0) {
	...
	for (int i = 0; i < tmpSize; i++) {
		...
		tmpByteCounter = decodeTRAJMessagePoint(&waypoint, copiedData.data(), debug);
		if (tmpByteCounter < 0) {
	...
	if (nPointsHandled == trajecoryHeader.nWaypoints) {
		std::cout << "TRAJ received; " << trajecoryHeader.nWaypoints << " points." << std::endl;
