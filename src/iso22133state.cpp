...
void ISO22133::State::handleEvent(TestObject& obj, const ISO22133::Events::EventType event) {
	...
	auto transition = std::find_if(language.begin(), language.end(), [&event, this](const ISO22133::Transition& tr) {
		return event == tr.event && this->getStateID() == tr.source;
	});
	if (transition == language.end()) {
		throw std::runtime_error(std::string("Unexpected event '") + Events::descriptions.at(event) + "' in state " +
								 this->getName());
	}
...
void ISO22133::State::handleOSEM(TestObject& obj, ObjectSettingsType& osem) {
	...
	msg.str(std::string());
	msg << "Set HEAB timeout to " << obj.heartbeatTimeout.count() << " ms. (" << 1000 / obj.heartbeatTimeout.count()
		<< " Hz) " << std::endl;
	std::cout << msg.str();

	obj.monrPeriod = std::chrono::milliseconds(1000 / (uint)osem.rate.monr);
	msg.str(std::string());
	msg << "Setting MONR period to " << obj.monrPeriod.count() << " ms. (" << 1000 / obj.monrPeriod.count() << " Hz) "
		<< std::endl;
	std::cout << msg.str();
...
void ISO22133::State::handleSTRT(TestObject& obj, StartMessageType& strt) {
	...
	auto currentTime =
	  std::chrono::to_timeval(std::chrono::system_clock::now().time_since_epoch() - obj.getNetworkDelay());
	...
	if (diffint > -250) {
		std::stringstream ss;
		ss << "Got STRT message with start time in " << diff.tv_sec << " seconds, " << diff.tv_usec
		   << " mySecs. Waiting" << std::endl;
		std::cout << ss.str();
		...
		ss << "Requested time: " << strt.startTime.tv_sec << " seconds, " << strt.startTime.tv_usec << " mySecs."
		   << std::endl;
