...
protected:
	...
	void SetUp() override {
		...
		auto offset				 = encodeTRAJMessageHeader(&header,
											   trajectoryID,
		...
		for (double i = 0; i < nTrajPoints; i++) { // Using float as loop counter to avoid ugly explicit casts
			...
			offset				  = encodeTRAJMessagePoint(
				 &tv, pos, spd, acc, curvature, points, sizeof(encodeBuffer) - (points - encodeBuffer), debug);
			ASSERT_GT(offset, 0);
			points += offset;
		}
		offset = encodeTRAJMessageFooter(points, sizeof(encodeBuffer) - (points - encodeBuffer), debug);
		ASSERT_GT(offset, 0);
		points += offset;
	}
