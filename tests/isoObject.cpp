...
public:
	SimulatedTestObject(int tcpSocket) :
	  TestObject(tcpSocket) {
		this->startHandleTCP();
...
public:
	...
	void connect() {
		try {
			tcpSocket.connect(
			  ip::tcp::endpoint(ip::address_v4::from_string(listenIP), ISO_22133_DEFAULT_OBJECT_TCP_PORT));
	...
	void sendHeartbeat(const ControlCenterStatusType ccStatus) {
		...
		auto nBytesWritten	  = encodeHEABMessage(&header,
					...
					while (data.size() > 0) {
						try {
							...
							if (obj != nullptr) {
								...
								std::memcpy(
								  address, ep.address().to_string().c_str(), ep.address().to_string().length());
								int handled = obj->handleUDPMessage(data.data(),
																	data.size(),
																	listener.getUDPSocket()->native_handle(),
																	address,
																	ep.port());
